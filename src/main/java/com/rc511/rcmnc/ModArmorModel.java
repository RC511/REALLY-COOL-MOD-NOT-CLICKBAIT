...
public class ModArmorModel extends BipedModel {
	...
	public ModArmorModel(float sizing)
	{
		super(sizing);
		this.texWidth = 64;
		this.texHeight = 32;
	...
	public void render(MatrixStack matStack, IVertexBuilder buffer, int packLight, int packOverlay, float red, float green, float blue, float alpha)
	{
		render(matStack, buffer, packLight, packOverlay, red, green, blue, alpha);
	}
	
	public void setRotationAngles(LivingEntity entity, float swing, float swingAmount, float tickAge, float headYaw, float headPitch)
	{
		setRotationAngles(entity, swing, swingAmount, tickAge, headYaw, headPitch);
	...
	public void setRotateAngle(ModelRenderer renderedModel, float x, float y, float z)
	{
		renderedModel.xRot = x;
		renderedModel.yRot = y;
		renderedModel.zRot = z;
	}
}
